...
class ContainersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create container" do
    assert_difference('Container.count') do
      post containers_url, params: { container: { command: @container.command, container_id: @container.container_id, image: @container.image, name: @container.name, port: @container.port, status: @container.status } }
    end
  ...
  test "should update container" do
    patch container_url(@container), params: { container: { command: @container.command, container_id: @container.container_id, image: @container.image, name: @container.name, port: @container.port, status: @container.status } }
