...
class ContainersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create container" do
    assert_difference('Container.count') do
      post containers_url, params: { container: { command: @container.command, containerid: @container.containerid, exposedport: @container.exposedport, hostport: @container.hostport, image: @container.image, name: @container.name, status: @container.status } }
    end
  ...
  test "should update container" do
    patch container_url(@container), params: { container: { command: @container.command, containerid: @container.containerid, exposedport: @container.exposedport, hostport: @container.hostport, image: @container.image, name: @container.name, status: @container.status } }
